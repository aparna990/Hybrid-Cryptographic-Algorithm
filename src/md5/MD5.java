...
public class MD5 {
	...
	/* The Hash function in bytes is received from func_md5 & converted to Hexadecimal representation */
		...
		public static void main(String[] args)
		{
			...
			String msg[]={"qwerty","zxcv","pwd123","test123","a","abc","Preethish Shetty","abcdefghijklmnopqrstuvwxyz","0123456789","ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", "$123#"};
		    System.out.println("\nFinal Result : ");
			for(String s:msg)
	...
	/* Test Code
		...
		FOR INPUT PREPROCESSING :
			-System.out.println("Total LenBytes:"+messageLenBytes);
		 	-System.out.println("Total Blocks:"+numBlocks);
			-System.out.println("Total Len:"+totalLen);

			1) +8   = accounts for the last 64 bits of padding which has the length
			2) >>>6 = to divide into chunks of 512
			3) <<6  = to have blocks of 64*8 bits
			4) paddingByte[0] = 0x80 => 10000000

		COPYING INPUT + PADDING INTO BUFFER :
		buffer[j >>> 2] = ((int)((index < messageLenBytes) ? message[index] : paddingZeroes[index - messageLenBytes]) << 24) | (buffer[j >>> 2] >>> 8);
			-First Step is to copy the message.
			-Then 0x80 (10000000) is shifted <<24 to make it in 32 bit format and the content of buffer is ORed with it 
			-Next since the padded value is zero the resulting effect is RIGHT SHIFT of Buffer and gets it down to required format (LITTLE ENDIAN)

		LENGTH PADDING :
			-The length values get stored in the Array starting from PaddingZero.length-8
			-Here the same process happens and finally the length is stored in LITTLE ENDIAN FORMAT
	
		TO DISPLAY TVALUES :
			for(int i=1;i<65;i++)
				System.out.println("T["+i+"] = "+ Integer.toHexString(Tval[i]));

		TO DISPLAY BUFFER VALUES :
		for(int p=0;p<16;p++)
				  System.out.println("\nbuffer["+p+"] = "+Integer.toBinaryString(buffer[p]));

		System.out.println("PZ = "+ paddingZeroes[55]);
    	System.out.println("PZ<<24 = "+ (paddingZeroes[55]<<24));

		TO DISPLAY PADDING VALUES :
		System.out.println("i = "+i+" & Message Len in Bits : "+messageLenBits);
		
		TO DISPLAY STANDARD BYTES & BIT VALUES :
			//System.out.println("Total LenBytes:"+messageLenBytes);
			//System.out.println("Total LenBits:"+Long.toBinaryString(messageLenBits));
			//System.out.println("Total Blocks:"+numBlocks);
			//System.out.println("Total Len:"+totalLen);
			//System.out.println("Padding Zeroes: "+paddingZeroes.length);
